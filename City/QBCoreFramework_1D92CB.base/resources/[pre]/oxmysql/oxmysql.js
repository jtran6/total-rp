...
function createCompiler(config) {
  ...
  function toArrayParams(tree, params) {
    ...
    for(const key in params) {
      parsedParams[key.charAt(0) == '@' ? key.substring(1) : key] = params[key]
    }

    const tokens = tree[1];
    for (let i=0; i < tokens.length; ++i) {
      if(typeof tokens[i] === 'number') {
        const param = parsedParams[(tokens[i] + 1).toString()];
        arr.push(param !== undefined ? param : null);
      } else {
        arr.push(parsedParams[tokens[i]] !== undefined ? parsedParams[tokens[i]] : null);
      }
    }
...
const { createPool } = __webpack_require__(/*! mysql2/promise */ "./node_modules/mysql2/promise.js");
const { ConnectionStringParser } = __webpack_require__(/*! connection-string-parser */ "./node_modules/connection-string-parser/dist/connection-string-parser.js");
