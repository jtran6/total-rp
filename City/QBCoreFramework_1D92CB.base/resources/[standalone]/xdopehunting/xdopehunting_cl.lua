...
Citizen.CreateThread(function()
	local StartBlip = AddBlipForCoord(-769.23773193359, 5595.621582031, 33.485713)
							...
							if PlyToAnimal < 2.0 then
								...
								if IsControlJustReleased(0, Keys['E']) then
									if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
										...
										QBCore.Functions.Notify('You need to use the knife!', "error")
									end
								end
...
function SlaughterAnimal(AnimalId)
	...
	QBCore.Functions.Notify('You slaughtered the animal and recieved ' ..AnimalWeight.. 'kg of meat', "success")

	TriggerServerEvent('QBCore:reward', AnimalWeight)

	DeleteEntity(AnimalId)
end

function SellItems()
	TriggerServerEvent('QBCore:sell')
end
