            ...
            if CanUseWeapon(allowedWeapons) then
                ...
                if closestTire ~= nil then
                    ...
                    if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
                        Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, tostring("[~r~E~w~] to Slash Tires"))
                        if IsControlJustPressed(1, 38) then

                            TriggerEvent('pogressBar:drawBar', 1100, 'Slashing Tire...')
                            ...
                            exports['mythic_notify']:DoShortHudText('error', 'Tire Slashed')
                        end
                    end
                end
            end
...
AddEventHandler("SlashTires:SlashClientTire", function(tireIndex)
    TriggerEvent('chat:addMessage', {
        template = '<div class="chat-message status">{1}</div>',
        args = {src, "^1A player is trying slash a tire"}
    });
    local player = PlayerId()
    local plyPed = GetPlayerPed(player)
